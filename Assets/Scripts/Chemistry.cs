...
public static class Chemistry
{
    ...
    public static void React()
    {
        ...
        int left = 0;
        foreach (var kv in pool) left += kv.Value.Count;

        lab.Recompute();
        if (made.Count == 0)
            lab.Say("Реакция прошла впустую: из этих атомов ни одного знакомого вещества не собрать.",
                new Color(1f, 0.85f, 0.6f));
        else
            lab.Say("Реакция: получилось " + string.Join(", ", made.ToArray()) +
                    (left > 0 ? ".  Осталось свободных атомов: " + left : "."),
